...
from utils.mc_utils import enable_cls_dropblock
...
def train(opt, device):
    ...
    with torch_distributed_zero_first(LOCAL_RANK), WorkingDirectory(ROOT):
        ...
        # Classification DropBlock #CLS
        n_cls_dropblock = enable_cls_dropblock(
            model,
            enabled=opt.cls_dropblock,
            gamma=opt.cls_dropblock_gamma,
            block_size=opt.cls_dropblock_size,
        )

        if RANK in {-1, 0} and opt.cls_dropblock:
            LOGGER.info(
                f"Classify DropBlock enabled on {n_cls_dropblock} Classify head(s): "
                f"gamma={opt.cls_dropblock_gamma}, block_size={opt.cls_dropblock_size}"
            )
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            # Forward
            with torch.amp.autocast("cuda", enabled=amp): # stability issues when enabled
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--cls-dropblock",
        action="store_true",
        help="enable DropBlock in YOLOv5 classification head before global average pooling",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.03,
        help="drop probability gamma for classification DropBlock",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="block size for classification DropBlock, use odd number such as 1, 3, 5",
    )
