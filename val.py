...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Loss
        if compute_loss and mc_samples == 1:
        ...
        with dt[2]:
            if mc_samples > 1:
                preds = mc_non_max_suppression(
                    ...
                    img_size=im.shape[-1],
                    return_extra=False,  # val.py는 기존 형식 [xyxy, conf, cls] 유지
                )
            else:
...
def parse_opt():
    ...
    parser.add_argument("--mc-var-thres", type=float, default=None, help="remove boxes with normalized loc std above threshold") #MC
