...
#CLS
def enable_cls_dropblock(model, enabled=True, gamma=0.03, block_size=3):
    """
    YOLOv5 classification head(Classify)에 DropBlock을 삽입/활성화한다.

    model:
        ClassificationModel 또는 checkpoint에서 로드된 YOLOv5-cls model

    enabled:
        True면 training 중 Classify head의 conv output에 DropBlock 적용

    gamma:
        drop center probability. 처음에는 0.01~0.03 권장.

    block_size:
        spatial block size. img=224 기준 마지막 feature가 작으므로 1 또는 3 권장.
    """
    from models.common import Classify, MCEdgeDropBlock2d
    ...
    for m in model.modules():
        if isinstance(m, Classify):
            found += 1
            m.use_cls_dropblock = enabled

            if not hasattr(m, "cls_dropblock"):
                m.add_module(
                    "cls_dropblock",
                    MCEdgeDropBlock2d(
                        gamma=gamma,
                ...
                m.cls_dropblock.gamma = gamma
                m.cls_dropblock.block_size = block_size
                m.cls_dropblock.always_on = False
